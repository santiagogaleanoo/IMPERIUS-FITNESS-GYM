...
export function HeroSection() {
  ...
  // 🔹 Botón principal — redirige a Productos Destacados si está autenticado
  const handleComienzaHoy = () => {
    if (isAuthenticated) {
      ...
      if (section) {
        ...
        console.warn("⚠️ No se encontró la sección 'productos-destacados'")
      }
    } else {
  ...
  // 🔹 Botón secundario — redirige a Instalaciones
  const handleVerInstalaciones = () => {
    const section = document.getElementById("instalaciones")
    if (section) {
      section.scrollIntoView({ behavior: "smooth" })
    } else {
      console.warn("⚠️ No se encontró la sección 'instalaciones'")
    }
  }
